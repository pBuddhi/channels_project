...
channel_routing = [
    route("websocket.connect", ws_connect, path=r"^/chat/room/(?P<room_id>[0-9_]+)/$"),
    route("websocket.receive", ws_message, path=r"^/chat/room/(?P<room_id>[0-9_]+)/$"),
    route("websocket.disconnect", ws_disconnect, path=r"^/chat/(?P<room_id>[0-9_]+)/$"),
]
